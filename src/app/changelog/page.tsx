...
const RELEASES: Release[] = [
  {
    version: '0.20.0',
    date: '2026-05-07',
    title: 'MVP handover · pre-launch coming-soon · quiz cascade',
    description: 'Generated from 1 commit on this date — see the project repo for full diffs.',
    changes: [
      { text: 'fix subtitle spacing + editorial cascade entrance', type: 'improved' as ChangeType },
    ],
  },
  {
    version: '0.19.0',
    date: '2026-05-06',
    title: 'Compliance, deactivate, JA copy, content polish, quiz icons',
    description: 'Generated from 24 commits on this date — see the project repo for full diffs.',
    changes: [
      { text: 'remove dataofjapan attribution caption — CC0 1.0 needs none', type: 'improved' as ChangeType },
      { text: 'real console errors — invalid path d, aria-hidden on focused subtree', type: 'fixed' as ChangeType },
      { text: 'remove Shimotakai-gun — irregular district listing', type: 'improved' as ChangeType },
      { text: '"Deactivate" replaces "Delete" — keep the record, auto-reactivate on sign-in', type: 'added' as ChangeType },
      { text: 'click-to-copy chip for Japanese place names + partner-site links', type: 'added' as ChangeType },
      { text: 'Revert "feat(areas): merge List + Map into one unified experience"', type: 'fixed' as ChangeType },
      { text: 'shoji card so kinu inputs are visible (no more white-on-white)', type: 'fixed' as ChangeType },
      { text: 'merge List + Map into one unified experience', type: 'added' as ChangeType },
      { text: 'T&C + Privacy ack — 11-section ToC, signup tickbox, audit log', type: 'added' as ChangeType },
      { text: 'land icon — diamond orientation, four parcels', type: 'improved' as ChangeType },
      { text: 'land icon — four irregular parcels with street gaps', type: 'improved' as ChangeType },
      { text: 'custom land icon (subdivided plot)', type: 'improved' as ChangeType },
      { text: 'custom kominka icon (chigi + gable + posts)', type: 'improved' as ChangeType },
      { text: 'replace 5-dot difficulty meter with speedometer gauge', type: 'improved' as ChangeType },
      { text: 'replace hand-drawn icons with Lucide', type: 'improved' as ChangeType },
      { text: 'modernise step cards, progress rail, page chrome', type: 'improved' as ChangeType },
      { text: 'add Family Composition question for municipal-culture fit', type: 'added' as ChangeType },
      { text: 'remove "we won\'t share with third parties" claim', type: 'fixed' as ChangeType },
      { text: 'filled-card inputs site-wide, modernise auth pages', type: 'improved' as ChangeType },
      { text: 'modernise page, form, and global Input style', type: 'improved' as ChangeType },
      { text: 'journey timeline UX for phase filter', type: 'improved' as ChangeType },
      { text: 'borderless filter pills, modern Ma Space treatment', type: 'improved' as ChangeType },
      { text: 'stop panel-height jitter on hover', type: 'fixed' as ChangeType },
      { text: 'freeze 3D map on scroll when InfoPanel grows past it', type: 'added' as ChangeType },
    ],
  },
  {
    version: '0.18.0',
    date: '2026-05-05',
    title: '3D map snow-country legend, sticky-on-scroll, polish',
    description: 'Generated from 8 commits on this date — see the project repo for full diffs.',
    changes: [
      { text: 'remove D2L journey stepper from /areas page', type: 'improved' as ChangeType },
      { text: 'backgroundless editorial legend, mobile visible', type: 'improved' as ChangeType },
      { text: 'nudge legend right so Sea of Japan label stays visible', type: 'fixed' as ChangeType },
      { text: 'relocate legend to top-left, drop redundant onboarding hint', type: 'improved' as ChangeType },
      { text: 'shrink legend, dock over Pacific (bottom-right)', type: 'improved' as ChangeType },
      { text: 'overlay snow-country legend, drop full 47-prefecture list', type: 'improved' as ChangeType },
      { text: 'bidirectional prefecture legend below 3D map', type: 'added' as ChangeType },
      { text: 'default prefecture borders to on', type: 'improved' as ChangeType },
    ],
  },
  {
    version: '0.17.0',
    date: '2026-05-04',
    title: 'Account redesign, area imagery, journey notes, sidebar sticky',
    description: 'Generated from 6 commits on this date — see the project repo for full diffs.',
    changes: [
      { text: 'show hero imagery in recommended-areas cards', type: 'added' as ChangeType },
      { text: 'preserve top breathing room when sidebar sticks', type: 'fixed' as ChangeType },
      { text: 'notes accordion shows real content for every phase, not just current', type: 'fixed' as ChangeType },
      { text: 'button padding, area-card border removal, override INSERT, hero CTA', type: 'fixed' as ChangeType },
      { text: 'modern editorial dashboard — single hero, refined sidebar', type: 'improved' as ChangeType },
      { text: 'use service client in cached getJourneyState', type: 'fixed' as ChangeType },
    ],
  },
  {
    version: '0.16.0',
    date: '2026-04-30',
    title: 'D2L journey integration — phases, signals, scoring, taxonomy seed',
    description: 'Generated from 47 commits on this date — see the project repo for full diffs.',
    changes: [
      { text: 'replace borders with shadow + ring patterns (Ma Space alignment)', type: 'improved' as ChangeType },
      { text: 'comprehensive D2L refinement — every surface modernised', type: 'improved' as ChangeType },
      { text: 'all steps clickable + modern pill-row Content Hub filter', type: 'fixed' as ChangeType },
      { text: 'modern + seamless redesign across all D2L surfaces', type: 'improved' as ChangeType },
      { text: 'hide chip on home + add slide-in transition to sheet', type: 'fixed' as ChangeType },
      { text: 'pin nav-map sheet to right edge with explicit full height', type: 'fixed' as ChangeType },
      { text: 'reserve descender room in directory card titles', type: 'fixed' as ChangeType },
      { text: 'area card title descender clipping + grid top spacing', type: 'fixed' as ChangeType },
      { text: 'trigger redeploy with newly-mirrored Preview env vars', type: 'improved' as ChangeType },
      { text: 'escape JSX apostrophes and move aria-disabled to non-li element', type: 'fixed' as ChangeType },
      { text: 'wire lint-no-direct-fetch into the lint-and-test job', type: 'improved' as ChangeType },
      { text: 'add lint guard against direct fetch() in journey module', type: 'improved' as ChangeType },
      { text: 'mount NavMapInline on /compare and /areas', type: 'added' as ChangeType },
      { text: 'mount NavMapChip in site header (md+ breakpoint)', type: 'added' as ChangeType },
      { text: 'add Markdown + PDF journey export', type: 'added' as ChangeType },
      { text: 'add ExternalBookmarks with SSRF-defended OG previews', type: 'added' as ChangeType },
      { text: 'add NotesByPhase with sanitized markdown editor', type: 'added' as ChangeType },
      { text: 'mount JourneyPhaseBlock at top of /account', type: 'added' as ChangeType },
      { text: 'add setPhaseOverride action, /api/journey/state, useJourneyState', type: 'added' as ChangeType },
      { text: 'add row-locking token bucket helper', type: 'added' as ChangeType },
      { text: 'mount PhaseFilterPrimaryRow + accept phase/buyerType filters', type: 'added' as ChangeType },
      { text: 'add PhaseFilterPrimaryRow component', type: 'added' as ChangeType },
      { text: 'add Buyer Type chip + preserve phase on clearAll', type: 'added' as ChangeType },
      { text: 'extend getFilteredArticles with phase + buyerType', type: 'added' as ChangeType },
      { text: 'add Backfill review desk pane (awaiting/tagged/untagged)', type: 'added' as ChangeType },
      { text: 'heuristic + idempotent migration script', type: 'added' as ChangeType },
      { text: 'wire red_flag_panel_viewed signal via /api/journey/signal', type: 'added' as ChangeType },
      { text: 'wire comparison_viewed signal on RSC visit', type: 'added' as ChangeType },
      { text: 'wire lead_submitted signal on Express Interest', type: 'added' as ChangeType },
      { text: 'wire area_saved signal on city save', type: 'added' as ChangeType },
      { text: 'wire quiz_started + quiz_completed signal capture', type: 'added' as ChangeType },
      { text: 'add captureSignalSafe non-blocking wrapper', type: 'added' as ChangeType },
      { text: 'seed 13 phase tags + 4 buyer-type tags', type: 'added' as ChangeType },
      { text: 'add phase + buyerTypes fields to article schema', type: 'added' as ChangeType },
      { text: 'register phaseTag + buyerTypeTag in schema index', type: 'added' as ChangeType },
      { text: 'add buyerTypeTag document schema', type: 'added' as ChangeType },
      { text: 'add phaseTag document schema', type: 'added' as ChangeType },
      { text: 'add cached RSC query getJourneyState', type: 'added' as ChangeType },
      { text: 'add captureSignal server-only API with inference + audit log', type: 'added' as ChangeType },
      { text: 'generate Database types and apply to service client', type: 'added' as ChangeType },
      { text: 'add service-role client for journey writes', type: 'added' as ChangeType },
      { text: 'add inference engine — pure monotonic-forward function', type: 'added' as ChangeType },
      { text: 'add per-phase \'Next:\' hint copy', type: 'added' as ChangeType },
      { text: 'add phase-mapping (11 backend → 6 user labels)', type: 'added' as ChangeType },
      { text: 'add core types — Phase, BuyerType, SignalType, JourneyState', type: 'added' as ChangeType },
      { text: 'add journey tables — state, signals, notes, bookmarks, og_cache', type: 'added' as ChangeType },
      { text: 'add runtime + dev deps for D2L module', type: 'improved' as ChangeType },
    ],
  },
  {
    version: '0.15.0',
